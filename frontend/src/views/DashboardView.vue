<template>
  <div class="p-4 sm:p-6 max-w-7xl mx-auto">
    ...
    <div class="mb-5 sm:mb-7">
      <h1 class="text-xl sm:text-2xl font-display font-bold text-zinc-100">Product Search</h1>
    ...
    <!-- Stats row -->
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-4 mb-5 sm:mb-7">
      <div class="stat-card">
    ...
    <div class="card p-4 sm:p-5 mb-5 sm:mb-6">
      <form @submit.prevent="runSearch" class="flex flex-col sm:flex-row gap-3">
        <div class="flex-1 min-w-0">
          <input
            v-model="keyword" type="text" class="input"
            placeholder='"wireless earbuds", "phone case"…'
            :disabled="loading" required />
        </div>
        <button
          type="submit" class="btn-primary sm:shrink-0 sm:px-8 justify-center"
      ...
      <div v-if="loading" class="mt-4">
        ...
        <div class="flex gap-2 flex-wrap">
          <span v-for="c in countries" :key="c.name"
            ...
            <span>{{ c.flag }}</span>
            <span class="hidden xs:inline">{{ c.name }}</span>
          </span>
        </div>
      </div>
      ...
      <div v-if="auth.remaining === 0 && !auth.isAdmin && !loading"
        class="mt-4 flex items-start gap-2 text-amber-400 text-sm bg-amber-400/10 border border-amber-400/20 rounded-xl px-4 py-3">
    ...
    <div v-if="searchError"
      class="mb-5 flex items-start gap-2 text-red-400 text-sm bg-red-400/10 border border-red-400/20 rounded-xl px-4 py-3">
      <svg class="w-4 h-4 flex-shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
    ...
    <div v-if="results.length > 0" class="animate-fadein">
      ...
      <div class="mb-4 space-y-3">
        <div class="flex items-center justify-between flex-wrap gap-2">
          <h2 class="font-display font-semibold text-zinc-100 text-sm sm:text-base">
            {{ sortedResults.length }} products
            <span class="text-zinc-500 font-normal">for "{{ lastKeyword }}"</span>
          </h2>
          <button @click="exportCSV" class="btn-secondary text-xs sm:text-sm">
        ...
        <!-- Filter + sort row — horizontal scroll on mobile -->
        <div class="flex gap-2 overflow-x-auto pb-1 -mx-4 px-4 sm:mx-0 sm:px-0 sm:flex-wrap">
          <select v-model="filterCountry"
            class="flex-shrink-0 bg-zinc-800 border border-zinc-700 text-zinc-300 text-xs sm:text-sm rounded-lg px-2.5 sm:px-3 py-1.5 focus:outline-none focus:border-brand-500">
      ...
      <div class="flex gap-2 overflow-x-auto pb-1 -mx-4 px-4 sm:mx-0 sm:px-0 sm:flex-wrap mb-4">
        <span class="flex-shrink-0 text-xs px-3 py-1.5 rounded-full bg-zinc-800 border border-zinc-700 text-zinc-400 font-display whitespace-nowrap">
          Avg: <span class="text-emerald-400 font-semibold ml-1">{{ avgProfit }}</span>
        </span>
        <span class="flex-shrink-0 text-xs px-3 py-1.5 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 font-display whitespace-nowrap">
          🟢 Low: <span class="font-semibold ml-1">{{ lowCompCount }}</span>
        </span>
        <span class="flex-shrink-0 text-xs px-3 py-1.5 rounded-full bg-amber-500/10 border border-amber-500/20 text-amber-400 font-display whitespace-nowrap">
          🟡 Med: <span class="font-semibold ml-1">{{ medCompCount }}</span>
        </span>
        <span class="flex-shrink-0 text-xs px-3 py-1.5 rounded-full bg-red-500/10 border border-red-500/20 text-red-400 font-display whitespace-nowrap">
      ...
      <div class="lg:hidden space-y-3">
        <div v-for="(p, i) in sortedResults" :key="i"
          class="bg-zinc-900/70 border border-zinc-800 rounded-2xl p-4 space-y-3">

          <!-- Rank + title + country -->
          <div class="flex items-start gap-3">
            <span class="text-xs text-zinc-600 font-mono w-5 flex-shrink-0 mt-0.5">{{ i + 1 }}</span>
            <div class="flex-1 min-w-0">
              <p class="text-sm text-zinc-200 font-medium leading-snug line-clamp-2">{{ p.title || '—' }}</p>
              <div class="flex items-center gap-2 mt-1.5 flex-wrap">
                ...
                <span :class="[
                  ...
                  p.competitionLevel === 'low' ? 'bg-emerald-500/10 text-emerald-400' :
                  p.competitionLevel === 'medium' ? 'bg-amber-500/10 text-amber-400' :
                  'bg-red-500/10 text-red-400'
                ]">
                  {{ p.competitionLevel === 'low' ? '🟢' : p.competitionLevel === 'medium' ? '🟡' : '🔴' }}
                  {{ p.competitionLevel }}
                </span>
              </div>
            </div>
          ...
          <!-- Price breakdown -->
          <div class="grid grid-cols-3 gap-2 bg-zinc-800/40 rounded-xl p-3">
            <div class="text-center">
              <p class="text-xs text-zinc-500 font-display mb-1">eBay Sold</p>
              <p class="font-mono text-blue-300 text-sm font-semibold">
                {{ currencySymbol(p.currency) }}{{ fmt(p.ebaySoldPrice || p.ebayPrice) }}
              </p>
            </div>
            <div class="text-center border-x border-zinc-700/50">
              <p class="text-xs text-zinc-500 font-display mb-1">AliExpress</p>
              <p class="font-mono text-zinc-300 text-sm">
                {{ currencySymbol(p.currency) }}{{ fmt(p.aliexpressPrice) }}
              </p>
            </div>
            <div class="text-center">
              <p class="text-xs text-zinc-500 font-display mb-1">Profit</p>
              <p class="font-mono text-sm font-semibold"
                :class="p.profit >= PREFERRED_MIN_PROFIT ? 'text-emerald-400' : p.profit > 0 ? 'text-amber-400' : 'text-red-400'">
          ...
          <div class="flex items-center justify-between gap-3">
            <span v-if="p.freeShipping || p.aliShippingCost === 0"
              class="flex items-center gap-1 text-emerald-400 text-xs font-display">
              <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/>
              </svg>
              Free shipping
            </span>
            <span v-else class="font-mono text-amber-400 text-xs">
              +{{ currencySymbol(p.currency) }}{{ fmt(p.aliShippingCost) }} ship
            </span>

            <div class="flex gap-2 ml-auto">
              <a :href="buildEbayLink(p)" target="_blank" rel="noopener noreferrer"
                class="text-xs bg-zinc-800 hover:bg-brand-500/20 hover:text-brand-400 border border-zinc-700 hover:border-brand-500/30 text-zinc-300 px-3 py-1.5 rounded-lg transition-all font-display">
                eBay ↗
              </a>
              <a :href="p.aliItemUrl || p.aliexpressUrl" target="_blank" rel="noopener noreferrer"
                class="text-xs bg-zinc-800 hover:bg-orange-500/20 hover:text-orange-400 border border-zinc-700 hover:border-orange-500/30 text-zinc-300 px-3 py-1.5 rounded-lg transition-all font-display">
                Ali ↗
              </a>
            </div>
          </div>
        ...
        <div class="text-xs text-zinc-500 font-display px-1 pb-2">
          Showing {{ sortedResults.length }} of {{ results.length }} products · SOLD listings only · last 30 days
        </div>
      </div>
        ...
        <table class="table-base">
          ...
          <tbody>
            <tr v-for="(p, i) in sortedResults" :key="i">
              ...
              <td>
                <span
                  :class="p.profit >= PREFERRED_MIN_PROFIT ? 'profit-positive' : p.profit > 0 ? 'text-amber-400 font-mono font-semibold' : 'profit-negative'"
    ...
    <!-- Empty after search -->
    <div v-else-if="!loading && searched" class="card p-10 sm:p-12 text-center">
      <div class="w-14 h-14 sm:w-16 sm:h-16 bg-zinc-800 rounded-2xl flex items-center justify-center mx-auto mb-4">
        <svg class="w-7 h-7 sm:w-8 sm:h-8 text-zinc-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <!-- Initial state -->
    <div v-else-if="!loading && !searched" class="card p-10 sm:p-12 text-center border-dashed">
      <div class="w-14 h-14 sm:w-16 sm:h-16 bg-brand-500/10 border border-brand-500/20 rounded-2xl flex items-center justify-center mx-auto mb-4">
        <svg class="w-7 h-7 sm:w-8 sm:h-8 text-brand-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
            d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/>
        </svg>
      </div>
...
function buildEbayLink(p) {
  ...
  return `${base}/sch/i.html?_nkw=${q}&LH_Sold=1&LH_Complete=1&LH_BIN=1&LH_ItemCondition=1000&LH_PrefLoc=1&_sop=10`
}
...
function normalise(p) {
  ...
  const margin = parseFloat(p.profitMarginPct ?? p.ProfitMarginPct) ?? (ebaySoldPrice > 0 ? parseFloat(((profit / ebaySoldPrice) * 100).toFixed(1)) : 0)
  return {
    title:            p.title            ?? p.Title            ?? '',
    country:          p.country          ?? p.Country          ?? '',
    currency:         p.currency         ?? p.Currency         ?? 'GBP',
    ebayPrice, ebaySoldPrice,
...
function startCountryAnimation() {
  ...
  countryInterval = setInterval(() => { idx = (idx + 1) % countries.length; currentCountry.value = countries[idx].name }, 1800)
}
...
async function runSearch() {
  ...
  loading.value = true; searchError.value = ''; results.value = []; searched.value = false
  filterCountry.value = ''; filterComp.value = ''
...
function exportCSV() {
  ...
  const headers = ['#','Title','Country','Currency','eBay Price','AliExpress Price','Ali Shipping','Profit','Free Shipping','Competition','eBay URL','Ali URL']
  const escape = v => { const s = String(v ?? ''); return s.includes(',') || s.includes('"') || s.includes('\n') ? `"${s.replace(/"/g,'""')}"` : s }
  const rows = data.map((p, i) => [i+1,p.title,p.country,p.currency,fmt(p.ebaySoldPrice||p.ebayPrice),fmt(p.aliexpressPrice),fmt(p.aliShippingCost),fmt(p.profit),p.freeShipping?'Yes':'No',p.competitionLevel,buildEbayLink(p),p.aliItemUrl||p.aliexpressUrl].map(escape).join(','))
  const csv  = [headers.join(','), ...rows].join('\n')
  const blob = new Blob(['\uFEFF' + csv], { type: 'text/csv;charset=utf-8;' })
  const url  = URL.createObjectURL(blob)
  const a    = document.createElement('a')
  const safe = lastKeyword.value.replace(/[^a-z0-9]/gi, '_').toLowerCase()
  a.href = url; a.download = `ebay_${safe}_${new Date().toISOString().slice(0,10)}.csv`
