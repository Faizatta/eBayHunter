  ...
  <div class="p-4 sm:p-6 max-w-7xl mx-auto">
    ...
    <div class="mb-6 sm:mb-8">
      <div class="flex items-center gap-3 mb-1">
        <div class="w-8 h-8 rounded-lg bg-brand-500/15 border border-brand-500/30 flex items-center justify-center flex-shrink-0">
          <svg class="w-4 h-4 text-brand-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
              d="M12 6V4m0 2a2 2 0 100 4m0-4a2 2 0 110 4m-6 8a2 2 0 100-4m0 4a2 2 0 110-4m0 4v2m0-6V4m6 6v10m6-2a2 2 0 100-4m0 4a2 2 0 110-4m0 4v2m0-6V4"/>
          </svg>
        </div>
        <h1 class="text-lg sm:text-xl font-display font-bold text-zinc-100 tracking-tight">Admin Panel</h1>
      </div>
      <p class="text-zinc-500 text-sm ml-11">Manage users, assign search credits, monitor usage</p>
    </div>

    <!-- Stats row -->
    <div v-if="stats" class="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3 mb-6 sm:mb-8 animate-fadein">
    ...
    <!-- Tabs -->
    <div class="flex gap-1 mb-5 bg-zinc-900 border border-zinc-800 rounded-xl p-1 w-full sm:w-fit overflow-x-auto">
      <button v-for="tab in tabs" :key="tab"
        :class="['px-3 sm:px-4 py-2 text-xs sm:text-sm font-display font-medium rounded-lg transition-all whitespace-nowrap flex-1 sm:flex-none',
          activeTab === tab
    ...
    <div v-if="activeTab === 'Users'" class="animate-fadein">
      ...
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
        <div class="relative flex-1 sm:flex-none">
          ...
          <input v-model="userSearch" type="text" placeholder="Search users…"
            class="input pl-9 w-full sm:max-w-xs" />
        </div>
        <div class="flex gap-2">
          <button @click="showCreateModal = true" class="btn-primary flex-1 sm:flex-none justify-center">
      ...
      <div v-else-if="!filteredUsers.length"
        class="flex flex-col items-center justify-center py-16 text-zinc-500 gap-3 bg-zinc-900/40 border border-zinc-800 rounded-2xl">
      ...
      <!-- ── DESKTOP: full table ── -->
      <div v-else class="hidden lg:block rounded-2xl border border-zinc-800 overflow-hidden bg-zinc-900/50">
            ...
            <tr v-for="u in filteredUsers" :key="u.id"
              ...
              <td class="px-5 py-4">
                <div class="flex items-center gap-3">
                  ...
                  <span class="font-mono text-sm text-zinc-200">{{ u.email }}</span>
                </div>
              </td>
              <td class="px-4 py-4">
              ...
              <td class="px-4 py-4">
                ...
                <div v-else class="flex flex-col gap-1.5">
                  ...
                  <div class="w-28 h-1.5 bg-zinc-800 rounded-full overflow-hidden">
                    <div class="h-full rounded-full transition-all duration-500" :class="pctColor(u)" :style="{ width: pct(u) + '%' }" />
                  </div>
                </div>
              </td>
              <td class="px-4 py-4">
                ...
                <div v-else class="flex items-center gap-1.5">
                  ...
                  <span class="font-mono text-sm font-semibold" :class="u.remaining > 0 ? 'text-emerald-400' : 'text-red-400'">{{ u.remaining }}</span>
                </div>
              </td>
              <td class="px-4 py-4">
              ...
              <td class="px-5 py-4">
                <div class="flex gap-2 items-center justify-end flex-nowrap">
                  <select :value="u.role"
                    class="bg-zinc-800 border border-zinc-700 text-zinc-300 text-xs rounded-lg px-2.5 py-1.5 focus:outline-none focus:border-brand-500 transition-all cursor-pointer"
                    @change="e => updateRole(u, e.target.value)">
                    <option v-for="r in roles" :key="r" :value="r">{{ r }}</option>
                  </select>
                  <template v-if="u.role !== 'Admin'">
                    <input v-model.number="u._topup" type="number" min="1" max="100000" placeholder="Credits"
                      class="w-20 bg-zinc-800 border border-zinc-700 text-zinc-300 text-xs rounded-lg px-2.5 py-1.5 focus:outline-none focus:border-emerald-500 transition-all placeholder-zinc-600"
                      @keydown.enter="topUp(u)" />
                    <button @click="topUp(u)"
        ...
        <div class="px-5 py-3 border-t border-zinc-800 bg-zinc-900/60 flex items-center justify-between">
          <span class="text-xs text-zinc-500 font-display">
            Showing <span class="text-zinc-300 font-medium">{{ filteredUsers.length }}</span> user{{ filteredUsers.length !== 1 ? 's' : '' }}
          </span>
          <span v-if="userSearch" class="text-xs text-zinc-600 font-display">Filtered by "{{ userSearch }}"</span>
        </div>
      </div>
      ...
      <div v-else class="lg:hidden space-y-3">
        <div v-for="u in filteredUsers" :key="u.id"
          class="bg-zinc-900/70 border border-zinc-800 rounded-2xl p-4 space-y-4">

          <!-- Top row: avatar + email + role + delete -->
          <div class="flex items-start gap-3">
            <div class="w-9 h-9 rounded-full flex items-center justify-center text-sm font-bold font-display flex-shrink-0"
              :class="avatarColor(u.role)">
              {{ u.email[0].toUpperCase() }}
            </div>
            <div class="flex-1 min-w-0">
              <p class="font-mono text-sm text-zinc-200 truncate">{{ u.email }}</p>
              <div class="flex items-center gap-2 mt-1">
                <span :class="['badge', 'badge-' + u.role.toLowerCase()]">{{ u.role }}</span>
                <span class="text-xs text-zinc-600 font-display">{{ fmtDate(u.createdAt) }}</span>
              </div>
            </div>
            <button v-if="u.role !== 'Admin'" @click="confirmDeleteUser = u"
              class="p-2 rounded-lg text-zinc-600 hover:text-red-400 hover:bg-red-500/10 transition-all flex-shrink-0">
              <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                  d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/>
              </svg>
          ...
          <div v-if="u.role !== 'Admin'">
            <div class="flex items-center justify-between mb-1.5">
              <span class="text-xs font-display text-zinc-500">Usage</span>
              <span class="text-xs font-mono text-zinc-400">{{ u.searchUsed }} / {{ u.searchLimit }}</span>
            </div>
            <div class="h-1.5 bg-zinc-800 rounded-full overflow-hidden">
              <div class="h-full rounded-full transition-all duration-500" :class="pctColor(u)" :style="{ width: pct(u) + '%' }" />
            </div>
            <div class="flex justify-between mt-1">
              <span class="text-xs font-display" :class="pctTextColor(u)">{{ pct(u) }}% used</span>
              <span class="text-xs font-mono font-semibold" :class="u.remaining > 0 ? 'text-emerald-400' : 'text-red-400'">
          ...
          <!-- Actions row -->
          <div class="flex gap-2 flex-wrap pt-1 border-t border-zinc-800/60">
            <select :value="u.role"
              class="flex-1 min-w-0 bg-zinc-800 border border-zinc-700 text-zinc-300 text-xs rounded-lg px-2.5 py-2 focus:outline-none focus:border-brand-500 transition-all cursor-pointer"
              @change="e => updateRole(u, e.target.value)">
              <option v-for="r in roles" :key="r" :value="r">{{ r }}</option>
            </select>
            <template v-if="u.role !== 'Admin'">
              <input v-model.number="u._topup" type="number" min="1" max="100000" placeholder="Credits"
                class="w-24 bg-zinc-800 border border-zinc-700 text-zinc-300 text-xs rounded-lg px-2.5 py-2 focus:outline-none focus:border-emerald-500 transition-all placeholder-zinc-600"
                @keydown.enter="topUp(u)" />
              <button @click="topUp(u)"
                class="text-xs bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-400 border border-emerald-500/20 px-3 py-2 rounded-lg transition-all font-display font-medium whitespace-nowrap">
                + Add
              </button>
            </template>
          </div>
        ...
        <div class="text-xs text-zinc-500 font-display px-1">
          {{ filteredUsers.length }} user{{ filteredUsers.length !== 1 ? 's' : '' }}
          <span v-if="userSearch"> · filtered by "{{ userSearch }}"</span>
        </div>
      </div>
    </div>
    ...
    <div v-if="activeTab === 'Recent Searches'" class="animate-fadein">
      ...
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
        ...
        <button @click="loadAll" class="btn-secondary self-start sm:self-auto">
          <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
              d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/>
          </svg>
      ...
      <div v-else-if="!filteredSearches.length"
        class="flex flex-col items-center justify-center py-16 gap-3 bg-zinc-900/40 border border-zinc-800 rounded-2xl">
        <div class="w-12 h-12 rounded-2xl bg-zinc-800 flex items-center justify-center">
          <svg class="w-6 h-6 text-zinc-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
              d="M21 21l-4.35-4.35M17 11A6 6 0 115 11a6 6 0 0112 0z"/>
      ...
      <div v-else class="space-y-4">
        <div v-for="group in groupedSearches" :key="group.email"
          ...
          <!-- Group header -->
          <div class="flex items-center gap-3 px-4 sm:px-5 py-3 sm:py-3.5 bg-zinc-900 border-b border-zinc-800 flex-wrap gap-y-1">
            ...
            <span class="font-mono text-sm text-zinc-200 font-medium truncate max-w-[160px] sm:max-w-none">{{ group.email }}</span>
            <span :class="['badge', 'badge-' + group.role.toLowerCase()]">{{ group.role }}</span>
            <span class="ml-auto text-xs text-zinc-500 font-display flex-shrink-0">
          ...
          <table class="hidden sm:table w-full border-collapse">
            ...
            <tbody class="divide-y divide-zinc-800/50">
              <tr v-for="(s, i) in group.searches" :key="s.id" class="hover:bg-zinc-800/20 transition-colors duration-100">
                ...
                <td class="px-4 py-3">
                  <span class="inline-flex items-center gap-1 bg-zinc-800/80 border border-zinc-700/60 text-zinc-400 text-xs font-mono font-medium rounded-lg px-2.5 py-1">
                    {{ s.resultCount ?? '—' }} <span class="text-zinc-600 font-display">results</span>
                  </span>
                </td>
                <td class="px-4 py-3">
                  <span v-if="s.parsedResults?.length" class="text-emerald-400 text-xs font-mono font-medium">
          ...
          <!-- ── Mobile: search rows as pills ── -->
          <div class="sm:hidden divide-y divide-zinc-800/50">
            <div v-for="(s, i) in group.searches" :key="s.id" class="px-4 py-3 flex items-center gap-3">
              <span class="text-xs text-zinc-600 font-mono w-5 flex-shrink-0">{{ i + 1 }}</span>
              <div class="flex-1 min-w-0">
                <p class="text-sm text-zinc-200 font-medium truncate">{{ s.keyword }}</p>
                <p class="text-xs text-zinc-500 font-display mt-0.5">{{ fmtDate(s.createdAt) }}</p>
              </div>
              <div class="flex flex-col items-end gap-1 flex-shrink-0">
                <span class="text-xs font-mono text-zinc-500">{{ s.resultCount ?? '—' }} results</span>
                <span v-if="s.parsedResults?.length" class="text-emerald-400 text-xs font-mono">
                  {{ adminBestProfit(s.parsedResults) }}
                </span>
              </div>
            </div>
    ...
    <transition name="modal">
      <div v-if="showCreateModal"
        ...
        <div class="card p-5 sm:p-6 w-full sm:max-w-md animate-fadein border border-zinc-700/80
          ...
          <div class="flex items-center justify-between mb-5 sm:mb-6">
            ...
            <button @click="closeCreate"
              class="w-8 h-8 flex items-center justify-center rounded-lg text-zinc-500 hover:text-zinc-300 hover:bg-zinc-800 transition-all flex-shrink-0">
              <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <div class="space-y-4">
            <div>
              ...
              <input v-model="cf.email" type="email" inputmode="email" class="input" placeholder="user@example.com" />
            </div>
            <div>
            ...
            <div v-if="createError"
              class="flex items-start gap-2 text-red-400 text-sm bg-red-400/10 border border-red-400/20 rounded-xl px-4 py-3">
    ...
    <transition name="modal">
      <div v-if="confirmDeleteUser"
        ...
        <div class="card p-5 sm:p-6 w-full sm:max-w-sm border border-zinc-700/80
          ...
          <p class="text-sm text-zinc-400 mb-6 leading-relaxed">
            ...
            <span class="text-zinc-200 font-medium font-mono bg-zinc-800 px-1.5 py-0.5 rounded text-xs break-all">
    ...
    <transition name="toast">
      <div v-if="toast"
        class="fixed bottom-20 sm:bottom-6 right-4 sm:right-6 text-sm font-display px-4 py-3 rounded-xl shadow-2xl flex items-center gap-2.5 z-40 border max-w-[calc(100vw-2rem)]"
        :class="toastErr
...
const groupedSearches = computed(() => {
  ...
  for (const s of filteredSearches.value) {
    ...
    const parsedResults = (() => { try { return JSON.parse(s.results ?? '[]') } catch { return [] } })()
...
function avatarColor(role) {
  const map = { Admin: 'bg-purple-500/20 text-purple-300', Pro: 'bg-brand-500/20 text-brand-300', Basic: 'bg-blue-500/20 text-blue-300', Free: 'bg-zinc-700 text-zinc-400' }
...
function pctColor(u) {
  const p = pct(u)
  return p >= 90 ? 'bg-red-500' : p >= 70 ? 'bg-amber-400' : 'bg-brand-500'
}
function pctTextColor(u) {
  const p = pct(u)
  return p >= 90 ? 'text-red-400' : p >= 70 ? 'text-amber-400' : 'text-zinc-500'
}
function fmtDate(d) {
...
async function createUser() {
  createError.value = ''
  if (!cf.value.email || !cf.value.password) { createError.value = 'Email and password are required.'; return }
  creating.value = true
  try {
...
function doDelete() {
  ...
  api.delete(`/admin/users/${u.id}`)
    .then(() => { users.value = users.value.filter(x => x.id !== u.id); showToast('User deleted'); loadAll() })
...
<style scoped>
.stat-decoration { @apply absolute bottom-0 right-0 w-16 h-16 rounded-tl-3xl bg-zinc-800/30 -z-0; }
